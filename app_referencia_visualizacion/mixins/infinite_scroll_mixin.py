...
class InfiniteScrollMixin:
    ...
    Uso:
        class MiVista(ctk.CTkFrame, InfiniteScrollMixin):
            def __init__(self, master):
                super().__init__(master)
                
                # Crear tu scroll_tabla normal
                self.scroll_tabla = ctk.CTkScrollableFrame(...)
                
                # Inicializar scroll infinito
                self.init_infinite_scroll(
                    scroll_widget=self.scroll_tabla,
                    lote_tamano=20
                )
            
            # Implementar método abstracto
            def render_fila_scroll(self, item, index):
                # Tu lógica para crear cada fila
                row = ctk.CTkFrame(self.scroll_tabla, ...)
                return row
    """
    ...
    def render_fila_scroll(self, item, index):
        ...
        Args:
            item: Elemento de la lista a renderizar
            index: Índice global del elemento (para colores alternados)
        
        Returns:
            No necesita retornar nada, solo crear el widget y empaquetarlo
        
        Ejemplo:
            def render_fila_scroll(self, item, index):
                bg = "#2d2d2d" if index % 2 == 0 else "#333333"
                row = ctk.CTkFrame(self._scroll_widget_inf, fg_color=bg)
                row.pack(fill="x", pady=1)
                ctk.CTkLabel(row, text=item["nombre"]).pack(side="left")
        """
    ...
    def get_total_registros(self):
        """Retorna el total de registros en cache"""
        return len(self._cache_datos_scroll)
    
    def get_registros_cargados(self):
        """Retorna cuántos registros se están mostrando actualmente"""
        return self._registros_cargados_scroll
