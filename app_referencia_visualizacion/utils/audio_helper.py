# app/utils/audio_helper.py (REEMPLAZAR COMPLETO - VERSIÓN MEJORADA)
...
class AudioHelper:
    ...
    def _beeps_sistema(self, cantidad=1, duracion=200, frecuencia=1000, pausa=100):
        ...
        Args:
            cantidad: Número de beeps
            duracion: Duración de cada beep en ms
            frecuencia: Frecuencia en Hz (37-32767)
            pausa: Pausa entre beeps en ms
        """
        if AUDIO_ENGINE == "winsound":
            import time
    ...
    def test_sonidos(self):
        """Método de prueba para verificar que los sonidos funcionen"""
        print("🔊 Probando sonidos...")
        print(f"Motor de audio: {AUDIO_ENGINE}")
        print(f"Carpeta de sonidos: {self.sounds_path}")
        
        print("\n1. Probando alerta de turno cruzado (3 beeps cortos)...")
        self.reproducir_alerta_turno_cruzado()
        
        import time
        time.sleep(2)
        
        print("\n2. Probando alerta de error (beep largo)...")
        self.reproducir_alerta_error()
